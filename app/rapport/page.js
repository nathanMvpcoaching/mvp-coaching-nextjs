    ...
    <>
      ...
      <main style={{ minHeight: '100vh', paddingTop: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 2 }}>
        <div style={{ textAlign: 'center', padding: '4rem' }}>
          ...
          <p style={{ color: 'rgba(232,240,245,0.3)', marginBottom: '1.5rem' }}>Aucun rapport disponible.</p>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '60px 5%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              ...
              <h1 style={{ fontFamily: 'Orbitron, monospace', fontWeight: 700, fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', color: '#fff' }}>
                {report.jeu} — Rapport de performance
              </h1>
            </div>
          ...
          {/* Score global */}
          <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: '0', marginBottom: '1.5rem', background: 'var(--dark2)', border: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '2.5rem 2rem', textAlign: 'center', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontFamily: 'Orbitron, monospace', fontSize: '4.5rem', fontWeight: 900, color: scoreColor(report.score_global), lineHeight: 1, display: 'block', textShadow: `0 0 30px ${scoreColor(report.score_global)}40` }}>{report.score_global}</span>
              <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.58rem', color: 'rgba(232,240,245,0.2)', letterSpacing: '0.12em', marginTop: '4px' }}>/100</span>
            </div>
            <div style={{ padding: '2.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', justifyContent: 'center' }}>
              <div style={{ fontSize: '0.92rem', color: 'rgba(232,240,245,0.5)', lineHeight: 1.65 }}>{report.resume}</div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                <span style={{ fontSize: '0.85rem' }}>⭐</span>
                <span style={{ fontSize: '0.85rem', color: 'rgba(232,240,245,0.5)', lineHeight: 1.4 }}><strong style={{ color: '#c8dde8' }}>Point fort — </strong>{report.point_fort}</span>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                <span style={{ fontSize: '0.85rem' }}>⚡</span>
                <span style={{ fontSize: '0.85rem', color: 'rgba(232,240,245,0.5)', lineHeight: 1.4 }}><strong style={{ color: '#ff8090' }}>Priorité — </strong>{report.priorite}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: 'var(--border)', border: '1px solid var(--border)', marginBottom: '1.5rem' }}>
            <div style={{ background: 'var(--dark2)', padding: '1.4rem' }}>
              ...
              <div style={{ fontSize: '0.88rem', color: 'rgba(232,240,245,0.5)', lineHeight: 1.5 }}>{report.point_fort}</div>
            </div>
            <div style={{ background: 'var(--dark2)', padding: '1.4rem' }}>
              ...
              <div style={{ fontSize: '0.88rem', color: 'rgba(232,240,245,0.5)', lineHeight: 1.5 }}>{report.priorite}</div>
            </div>
          </div>
            ...
            {(report.modules || []).map((mod, i) => (
              <div key={i} style={{ background: 'var(--dark2)', padding: '1.8rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  <div>
                    <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.52rem', color: 'rgba(232,240,245,0.2)', marginBottom: '4px' }}>MODULE 0{i + 1}</div>
                ...
                <div style={{ height: '2px', background: 'rgba(255,255,255,0.05)', marginBottom: '1.2rem', borderRadius: '1px' }}>
                  <div style={{ height: '100%', width: `${mod.score}%`, background: scoreColor(mod.score), borderRadius: '1px', transition: 'width 1s ease' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
                  {(mod.points || []).map((p, j) => (
                    <div key={j} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      <span className={`pill-${p.type}`} style={{ marginTop: '2px', whiteSpace: 'nowrap' }}>{p.type}</span>
                      <span style={{ fontSize: '0.82rem', color: 'rgba(232,240,245,0.45)', lineHeight: 1.5 }}>{p.texte}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ background: 'var(--dark2)', border: '1px solid var(--border)', padding: '2rem', position: 'relative', marginBottom: '3rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {(report.plan_semaine || []).map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '32px 1fr', gap: '12px', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: '0.88rem', color: 'rgba(232,240,245,0.55)', lineHeight: 1.55 }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '2rem', border: '1px dashed rgba(0,245,255,0.12)' }}>
            <p style={{ color: 'rgba(232,240,245,0.3)', marginBottom: '1.2rem', fontSize: '0.9rem' }}>Analyse une autre replay pour suivre ta progression</p>
